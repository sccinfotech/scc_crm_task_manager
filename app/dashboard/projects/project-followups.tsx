...
export function ProjectFollowUps({
  ...
  const getInitialEditData = (): Partial<ProjectFollowUpFormData> | undefined => {
    ...
    return {
      ...
      next_follow_up_date: selectedFollowUp.next_follow_up_date || undefined,
      ...
      <div className={`h-full flex flex-col bg-white rounded-2xl border border-slate-200 shadow-sm ${className}`}>
                      ...
                      <div className="mt-2 space-y-2">
                        {followUp.follow_up_date && (
                          <div className="px-2 py-0.5 rounded-lg bg-slate-50 border border-slate-100 inline-flex items-center gap-1.5">
                            ...
                            <span className="text-[9px] font-bold uppercase tracking-wider text-slate-600">
                              Follow-Up: {formatDateOnly(followUp.follow_up_date)}
                            </span>
                          </div>
                        )}
                        {followUp.next_follow_up_date && (
                          <div className="px-2 py-0.5 rounded-lg bg-slate-50 border border-slate-100 inline-flex items-center gap-1.5">
                            ...
                            <span className={`text-[9px] font-bold uppercase tracking-wider ${getFollowUpDateColor(followUp.next_follow_up_date)}`}>
                              Next: {formatDateOnly(followUp.next_follow_up_date)}
                            </span>
                          </div>
                        )}
                      </div>
        ...
        <div className="p-3 border-t border-slate-100 bg-white/60 rounded-b-2xl">
          {canWrite ? (
            ...
            >
              <div className="mb-3">
                <p className="text-xs text-slate-500 mb-2">
                  Add a note or set a follow-up date (at least one is required)
                </p>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-12 gap-4 items-start">
                ...
                <div className="sm:col-span-6">
                  <input
                    type="date"
                    name="follow_up_date"
                    className="w-full rounded-2xl border border-slate-200 bg-white/80 px-4 py-2.5 text-sm text-slate-700 shadow-sm transition-all duration-300 focus:border-cyan-500 focus:outline-none focus:ring-4 focus:ring-cyan-500/10"
                  />
                </div>
                <div className="sm:col-span-6">
                  <input
            ...
            <div className="rounded-xl bg-slate-50 border border-slate-200 p-3 text-sm text-slate-500">
              You have read-only access to follow-ups.
            </div>
          )}
        </div>
      </div>

...
import {
  ...
  deleteQuotationRequirement,
  getQuotationRequirementUploadSignature,
  getQuotationRequirements,
  updateQuotationRequirement,
  type QuotationPricingType as PricingType,
...
function RequirementDeleteModal({
  ...
  isOpen: boolean
  requirement: QuotationRequirement | null
  onClose: () => void
  onConfirm: () => void
  isDeleting: boolean
}) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true">
      ...
      <div className="relative z-10 w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
        ...
        <p className="mt-2 text-sm text-slate-600">
          Are you sure you want to delete this requirement? This will remove it from the quotation scope list.
        </p>
...
function RequirementModal({
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    let milestonePayload: QuotationRequirementFormData["milestones"] | undefined
    ...
    const result =
      mode === "create"
        ? await createQuotationRequirement(quotationId, payload)
        : await updateQuotationRequirement(requirement?.id ?? "", payload)
...
export function QuotationRequirements({
  ...
  className = "",
  isActiveTab = true,
}: QuotationRequirementsProps) {
  const router = useRouter()
  const { error: showError, success: showSuccess } = useToast()
  const [requirements, setRequirements] = useState<QuotationRequirement[]>([])
  ...
  const fetchRequirements = async (options?: { silent?: boolean }) => {
    ...
    setSummary(computeSummary(result.data || []))
    setLoading(false)
  }
  ...
  const handleOpenEdit = (requirement: QuotationRequirement) => {
    if (!canWrite) {
  ...
  const handleOpenDelete = (requirement: QuotationRequirement) => {
    if (!canWrite) {
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true)
    const result = await deleteQuotationRequirement(selectedRequirement.id)
    setDeleting(false)
    if (result.error) {

...
## Work Status (Start/Hold/End) Performance

- **`updateMyProjectWorkStatus`** – Returns `UpdateMyWorkStatusSupplement` (team_members + team_member_time_events) instead of full `Project`. Client merges into existing project state.
- **No `router.refresh()`** after work status update – UI updates via `setProject` merge; avoids redundant full page re-fetch.
- **Parallel checks** – `isUserAssignedToProject` and `select work_status` run in parallel for faster validation.
